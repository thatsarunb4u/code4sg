...
let createComment = async (input_json) => {
  let conn;
	try {
		...
		if (input_json.hasOwnProperty('parentCommentID')) {
			const resp = await conn.query(
				'INSERT into comment (body, postID, authorID, isAnonymous, parentCommentID) VALUES ( ?, ?, ?, ?, ?)',
				[input_json['body'], input_json['postID'], input_json['authorID'], input_json['isAnonymous'], input_json['parentCommentID']]
			);
			console.log(resp);
			...
			const resp = await conn.query(
				'INSERT into comment (body, postID, authorID, isAnonymous) VALUES ( ?, ?, ?, ?)',
				[input_json['body'], input_json['postID'], input_json['authorID'], input_json['isAnonymous']]
			);
			console.log(resp);
...
export { createComment, searchComments, flagComment, deleteComment, upvoteComment, downvoteComment };
