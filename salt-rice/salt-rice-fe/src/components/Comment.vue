			...
			<div class="profile-comment-action">
				<div class="actionPostSection responsive-marginpost">
					<div class="margin-right" @click="upVote">
						<svg class="icon" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 31.367 30.711">
					...
					<div class="innerflex-7" @click="downVote">
						<div>
							<svg class="icon" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 29.904 27.416">
            ...
            >
              ...
              <g>
				...
				<div class="replay-comment">
					<skeleton v-if="loading" width="5%" />
					<a v-else @click="isReplying = !isReplying" class="actionButtonClear inline-flex cursor">
						REPLY
					</a>
				</div>
			</div>
			<SubComment
				v-for="comment in comment.subComment"
					...
					<div class="new-comment">
						...
						<div class="profile-comment">
							<form action="#" @submit.prevent>
								<textarea id="textarea" placeholder="Add your comment" v-model="replyComment" />
...
export default {
	...
	components: {
		...
		SubComment: () => import('./SubComment.vue'),
	},
	data() {
		return {
			isReplying: false,
	...
	methods: {
      ...
      this.$emit("subReply", replyDict);
		...
		async deleteComment() {
			await protectedFetch(`${process.env.VUE_APP_BASE_API}/post/comment/${this.comment.commentID}`, {
				method: 'DELETE',
			});
