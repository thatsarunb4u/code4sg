<template>
	<section id="postSection">
		...
		<article>
			...
			<div class="actionPostSection">
				<div class="border margin-right">
					<svg @click="upVote" class="icon" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 31.367 30.711">
				...
				<div class="innerflex-7 border">
					<div>
						...
						>
							<g id="Group_98_1" data-name="Group 98" transform="translate(21.182 0)">
								<g id="Group_97_1" data-name="Group 97" transform="translate(0)">
									<path
										id="Path_167_1"
										data-name="Path 167"
							...
							<g id="Group_100_1" data-name="Group 100" transform="translate(0 0.004)">
			...
			<div>
				<div class="margin-top">
					<a href="#" class="actionButtonClear inline-flex">
						<img src="../assets/images/comment.svg" alt="icon comment" class="icon" />
						<span>REPLY</span>
					</a>
					<a href="#" class="actionButtonClear inline-flex">
						<img src="../assets/images/share.svg" alt="icon comment" class="icon" />
						<span>SHARE</span>
					</a>
				</div>
				<p>
				...
				<p>
					...
					<template v-else>
						Posted by
						<router-link :to="`/user/${author.userID}`" class="font-yellow" tag="a">{{
							author.nickname
						}}</router-link>
					</template>
				</p>
		...
		<div class="comments">
			<div class="margin-bottom-layout">
				...
				<div class="new-comment">
					...
					<div class="profile">
						<div>
							<img src="@/assets/images/iconfinder_1_avatar_2754574.svg" alt="" />
						</div>
						<div>
		...
		<Popup v-if="popup" :popupMsg="popupMsg" @popupClose="popup = false" />
	</section>
</template>
...
async function getPostData(postID) {
	const post = await (await protectedFetch(`${process.env.VUE_APP_BASE_API}/post/bypostid/${postID}`)).json();
	const author = await (await protectedFetch(`${process.env.VUE_APP_BASE_API}/user/byID/${post.authorID}`)).json();
	...
	methods: {
      ...
      console.log(this.popup);
			if (store.getters.isLoggedIn) {
				try {
					this.post.upVote++;
					await protectedFetch(`${process.env.VUE_APP_BASE_API}/post/${this.post.postID}/upvote`);
        ...
        this.popupMsg = "To access upvote. Please login or register first!";
        this.popup = true;
			}
		},
		async downVote() {
      if (store.getters.isLoggedIn) {
			try {
				this.post.downVote++;
				await protectedFetch(`${process.env.VUE_APP_BASE_API}/post/${this.post.postID}/downvote`);
				const { post, author } = await getPostData(this.$route.params.id);
        ...
        this.popupMsg = "To access downvote. Please login or register first!";
        this.popup = true;
			}
		},
		async reply() {
      if (store.getters.isLoggedIn) {
        ...
        this.popupMsg = "To access reply. Please login or register first!";
        this.popup = true;
			}
		},
      ...
      if (store.getters.isLoggedIn) {
		...
		async flag() {
			await protectedFetch(`${process.env.VUE_APP_BASE_API}/post/${this.post.postID}/flag`);
		},
	},
	computed: {
		updatedAtCalendar() {
